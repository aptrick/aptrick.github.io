...
function LoadEndorsements(callback) {
  d3.csv('data/endorsements.csv', function(error, data) {
    if (error) {
      callback(error)
    } else {
      data = data.filter(function(d) {
        return d.lat != ''; // && d.e2012 === 'Barack Obama';
      }).map(function(d) {
        d.lat = +d.lat;
        d.lng = +d.lng;
...
function FormatCirculation(val) {
  val = 1000 * Math.round(val / 1000);
  return 'circulation: ' + val.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
}
...
function Map(selector) {
  ...
  // Set height and width of SVG canvas. 
  this.svgWidth = $(selector + ' .svg-wrapper').width();
...
Map.prototype.tabulateEndorsements = function() {
  var counts = {};
      list = [];
  for (var i = 0; i < this.endorsements.length; i++) {
    var candidate = this.endorsements[i].e2016;
    if (counts.hasOwnProperty(candidate)) {
      counts[candidate] += 1;
    } else {
      counts[candidate] = 1;
    }
  ...
  for (candidate in counts) {
    list.push([candidate, counts[candidate]]);
  }
...
Map.prototype.lockFilter = function(candidate) {
  ...
  if (this.lockedFilter === candidate) {
    delete this.lockedFilter;  
  } else {
    this.lockedFilter = candidate;
    this.bubbles.selectAll('g').filter(function(d) {
      return d.e2012 !== candidate;
    })
      .classed('hidden', true);
    this.filterOptions.filter(function(d) {
      return d === candidate;
    })
      .classed('locked', true);
  }
...
Map.prototype.lockCandidate = function(candidate) {
  ...
  if (this.lockedCandidate === candidate) {
    delete this.lockedCandidate;  
  } else {
    this.lockedCandidate = candidate;
    this.bubbles.selectAll('g').filter(function(d) {
      return d.e2016 !== candidate;
    })
      .classed('hidden', true);
    this.legendItems.filter(function(d) {
...
Map.prototype.drawLegend = function() {
  ...
  this.filterOptions = d3.select('#legend2012')
    .selectAll('.legend-item').data(candidates2012).each(function(candidate) {

    var item = d3.select(this);
    
    item.select('.legend-label').text(candidate);
...
Map.prototype.updateInfoBox = function(datum) {
  ...
  // Update blurb. 
  var blurb = this.infoBox.select('.blurb');
  blurb.selectAll('p').remove();
  if (datum.blurb != '') {
    var paragraphs = datum.blurb.split('\\n');
...
Map.prototype.drawBubbles = function(endorsements) {
  var that = this;

  endorsements = endorsements.map(function(d) {
    d.x = that.projection([d.lng, d.lat])[0];
    d.y = that.projection([d.lng, d.lat])[1];
    d.r = that.size(Math.sqrt(d.circulation));
    return d;
  });

  this.endorsements = endorsements;

  var groups = this.bubbles.selectAll('g')
    .data(endorsements)
    .enter()
    .append('g')
  ...
  groups
    ...
    .transition()
    .duration(500)
    .delay(function(d) { return 50 + 100 * Math.random(); })
  ...
  // this.bubbles.selectAll('circle').on('mouseover', function() {
  //   var sel = d3.select(this);
  //   var d = sel.datum();
  //   console.log(d);
  //   sel.attr('r', 3 + that.size( Math.sqrt(d.circulation)));
  // }).on('mouseout', function() {
  //   var sel = d3.select(this);
  //   var d = sel.datum();
  //   sel.attr('r', that.size(Math.sqrt(d.circulation)));
  // });
};


$(document).ready(function() {
  mainMap = new Map('#map-main');
  var romneyMap = new Map('#map-repub');
  var obamaMap = new Map('#map-dem');
  maps = [mainMap, romneyMap, obamaMap];

  var q = d3.queue();
  q.defer(LoadStateShapes);
  q.defer(LoadEndorsements);
  q.await(function(error, us, endorsements) {
    if (error) throw error;
    for (var i = 0; i < maps.length; i++) {
      maps[i].drawMap(us);
    }
    mainMap.drawBubbles(endorsements);
    mainMap.drawLegend();
    // obamaMap.drawBubbles(endorsements.filter(function (d) {
    //   return d.e2012 === 'Barack Obama';
    // }));
    // romneyMap.drawBubbles(endorsements.filter(function (d) {
    //   return d.e2012 === 'Mitt Romney';
    // }));
  });
